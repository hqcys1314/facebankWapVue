    ...
    <div>
      ...
      <section class="wh_container" >
        <div class="wh_content_all" ref="ced">
          ...
          <div class="wh_content" v-if="isShowMonth">
          ...
          <div class="wh_content" v-else>
           <div class="wh_content_item" v-for="(item,index) in listWeek" @click="clickDay(item,index)">
              <div class="wh_item_date2" v-bind:class="[{wh_other_dayhide:false},
              {wh_chose_finish: item.chooseDay&&setClassFinish(item)},
              {wh_chose_ing:item.chooseDay&&setClassIng(item)},
              {wh_chose_pre:item.chooseDay&&setClassPre(item)},
              {wh_chose_com:item.chooseDay&&setClassCom(item)},
              ...
              {cal_color_ing:setClassIng(item)},
              {cal_color_pre:setClassPre(item)},
              {cal_color_com:setClassCom(item)}]"> {{item.id}}
              </div>
           </div>
          </div>
...
export default {
  ...
  created() {
    ...
    for(var i=2018;i<=this.myDate.getFullYear()+3;i++){
        this.year.push(i)
    }
