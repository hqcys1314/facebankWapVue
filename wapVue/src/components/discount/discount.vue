<template>
    <div v-cloak>
        <div class="item_box">
            <a href="javascript:;">
                <p @click="showCouponLayer()">
                    ...
                    <span class="estProfix" v-if="availableCouponCount > 0 && selectedCoupon.estimateProfix">+{{currency(selectedCoupon.estimateProfix)}}</span>
                    <small v-if="!userIsLogin">查看优惠券</small>
                    <small v-else-if="(couponCardList.length > 0 || redbagListData.length > 0) && availableCouponCount == 0 && userIsLogin">查看优惠券</small>
                    <small v-else-if="(couponCardList.length == 0 && redbagListData.length == 0) && availableCouponCount == 0 && userIsLogin">无可用卡券</small>
                    <small v-else-if="availableCouponCount > 0 && selectedCoupon =='' && userIsLogin">您有 <i>{{availableCouponCount}}</i> 张优惠券可用</small>

                    <!-- <small v-else-if="availableCouponCount == 0 && !userIsLogin">当前无可用卡券</small> -->
                    <small class="selected" v-if="availableCouponCount > 0 && selectedCoupon.bonusModel && userIsLogin">
                        {{selectedCoupon.bonusModel*1 == 1 ? ('返现红包+'+ selectedCoupon.bonusValue +'元') : ('加息红包+'+ selectedCoupon.bonusValue +'%')}}
                    </small>
                    <small class="selected" v-if="availableCouponCount > 0 && selectedCoupon.card_type && userIsLogin">
                        {{
        ...
        <div class="layer_coupon" :style="'z-index:'+ (showCoupon ? '1000' : '-1') +';opacity:'+(showCoupon ? '1' : '0')">
            <div class="contentBox" :style="'top:'+(showCoupon ? '4.25rem' : '100%')">
                <div class="couponHeader">
                    <a href="javascript:;" class="close" @click="showCoupon = false"></a>
                    <a href="javascript:;" class="notuse" @click="notUseCoupon">不使用</a>
                    <span>选择优惠券</span>
                </div>
                <div class="coupon_box" @touch.prevent>
...
export default {
  name: 'discount',
  props: ['curentSelectCoupon', "investAmount", "proInfo", "tremDayCal", "productType", "agreementState","nowDay","firstRepaymentDate", 'noAgreementLayer', 'isDeposit', 'depositProInfo', 'isLogin'],
  ...
  created:function(){
      this.userIsLogin = this.isLogin;
      this.amount = this.investAmount;
  },
  methods:{
      currency(number) {
          return Number((typeof number == 'string' ? number.replace(/,/g, '')*1 : number)).toFixed(2).replace(/\d{1,3}(?=(\d{3})+(\.\d*)?$)/g, '$&,');
      },
      //隐藏小提示层
      hideSTips(){
      ...
      showCouponLayer(item){
          ...
          if(!this.isLogin){
              this.utils.toLogin(window.location.pathname + window.location.search);
              return;
          }
          if(this.couponCardList.length == 0 && this.redbagListData.length == 0){
              return false;
          }
              var _this = this;
      ...
      filterCouponData (){
          this.couponCardListData.length = 0;
          if(this.couponCardList.length <= 0) return false
          let _this = this;
          let trueData = [], falseData = [];
          for(let i=0;i<this.couponCardList.length;i++){
              ...
              let productIf = items.order_type.indexOf(this.productType) > -1,
                  ...
                  amountIf = this.amount * 1 >= items.min_money*1,
                  time = items.order_term+'天',
                  money = this.currency(items.min_money*1 - this.amount*1);
      ...
      filterRedpackData(){
          ...
          for(let i=0; i<this.redbagList.length; i++){
              ...
              let amountIf = this.amount * 1 >= items.bonusEnableMoney*1,
                  ...
                  money = this.currency(items.bonusEnableMoney*1 - this.amount*1);
              //items['enable'] = (amountIf && tremDayIf && productIf && dateIsEffective);
              items['enable'] = (amountIf && tremDayIf && productIf);
              //if(dateIsEffective){
                  if(amountIf){
      ...
      initialization : function(){
          ...
          $.when(this.getRedPackList(), this.getCouponCardList()).then(function(redPackRes, couponCardRes){
              ...
              if(couponCardRes.code*1 == 1){
                  ...
                  if(_this.isDeposit*1 == 0){
                      _this.couponCardList = [];
                      _this.couponCardCount = 0;
                  }else{
                      _this.$parent.showLayer(couponCardRes.message);
                  }
              }
              _this.redbagListData.length = 0;
              _this.couponCardListData.length = 0;
  ...
  watch : {
      ...
      availableCouponCount(val){
          ...
          if(val > 0 && cookie != 'showed' && this.selectedCoupon == ''){
              this.showSTips = true;
          }else{
              this.showSTips = false;
          }
