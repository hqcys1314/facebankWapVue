<template>
  <div class="orderList">
      ...
      <div class="order_list" ref="list_box">
          ...
          <ul>
              <li v-for="item in getRecordListData">
                  <router-link :to="{ path: '/subjectInvestRecordDetail', query: {orderId : item.orderId, creditId : item.creditId, isOldSys : item.isOldSys} }">
                      <div class="content">
                          <div class="title">
                              {{item.productName}}
                  ...
                  <div class="tips_area" ref="listTips" v-if="item.orderState=='6'&&item.newestReport && item.newestReport.bizType == '4'" @click="showDetailLayer(item)">{{listTips!='' ? listTips : item.newestReport.subTitle}} <span class="icon icon_tips"></span></div>
                  <div class="tips_area" v-else-if="item.orderState=='6'&&item.newestReport" @click="showDetailLayer(item)">{{item.newestReport.subTitle}}，{{item.newestReport.businessTimeDesc}} <span class="icon icon_tips"></span></div>
              </li>
          </ul>
      ...
      <div class="layer_style" id="orderLayer" v-if="orderDetailLayerShow && currentOrder">
          ...
          <div class="layer_title" v-if="currentOrder.bizType == '4'">第{{currentOrder.body.repayCount*1+1}}期还款已完成</div>
  ...
  export default {
    ...
    data() {
      return {
          ...
          curOrderRank : 0,
          orderTypeFont: ['系统出借中', '计息中', '已结清', '流标', '提前还款','债权转让中','债权转让已完成'],
    ...
    methods: {
        ...
        showDetailLayer : function(item){
            ...
            if(item.newestReport.bizType == '4' && item.newestReport.body.nextRepaymentDate){
                this.listTips = '下个回款日：'+item.newestReport.body.nextRepaymentDate;
            }else{
                this.listTips = ''
            }
    ...
    //     }
    // },
    //components: {subjectInvestRecordLi}
