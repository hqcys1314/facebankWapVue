<template>
  <div class="content" style="background:#eaeaea;">
    ...
    <div  @touchstart="_touchStart"  @touchend="_touchEndUp" style="height:28rem;">
        <calendar @monthListLength="monthListLength" @changeShowWeekStatus="changeShowWeekStatus" ref="Calendars"   agoDayHide=1530115200  :isShowMonth="isShowMonthV"
...
<style src="./repaymentCalendar.less" lang="less"></style>
<script>
...
export default {
  data() {
    return {
      ...
      searchMonthV:'',
      searchDayV:'',
      stateMoneyV:'1',// state: 0,待还款;1,还款中;2, 已还款 3;无还款
      isShowMonthV:true,
      wapMcStatus:false,
  ...
  mounted() {
    ...
    //端上用2018/12/1
    if(this.$route.query.changeMonth){
      this.$refs.Calendars.ChoseMonth(this.$route.query.changeMonth);

    }

  },
  components: {
    calendar,calContent,wapTkInvest,informationLayer,layerWithImg,loading
  },
  watch:{
  ...
  methods: {
    ...
    showHelpContent(){
      ...
      this.informationLayerContent = '1. 刚购买的产品，在日历上没有显示？<br />' +
        '由于您刚购买的产品，还未满标，无法精确地计算出还款日期，需要等产品计息后才会在日历上显示。<br />' +
        '2.	我只购买了一款产品，为什么日历上一个月的还款有两天？<br />' +
        '由于您购买的智能推荐，起息后有多个债权，起息时间不一致，因此一个智能推荐的订单，会在月内分几天还款。<br />' +
        '3. 为什么预计还款日是今天，但今天没还款？<br />' +
        '因为银行结算等原因，您的债权可能会在下一个工作日还款到账。<br />' +
        '4.	还款日历的开始时间为什么是18年4月？<br /> ' +
        '为方便数据统计，还款日历的初始时间设定为18年4月。<br /> ' +
        '5. 为什么还款日历上的日期有还款，流水上没有？<br />' +
        '还款日历都是根据【约定还款的日期】展示日期的，而不是流水上【实际发生还款】的时间，由于银行结算等原因发生的还款跨天，不会在新的日期更新还款记录。<br />' +
        '6. 为什么还款日历上有的日期只有红包的还款记录？<br />' +
        '某些产品由于流标、提前还款等原因，没有债权还款了，但红包仍会在每期给您还款。<br />' +
        '7. 为什么有一个产品的债权在同一天还了好多期？<br />' +
        '当债权在还款日发生了提前还款，那么该日的提前还款部分会记录该债权剩余期（不包含当期）的所有金额，而当期还款会记录在应还的部分里。';
    },
...
//      this.calContentTop=this.$refs.Calendar.$el.clientHeight+'px';
              ...
              if(res.code==1){
                ...
                _this.isCctShowD=false;
                  ...
                  if(res.data){
                    ...
                    //_this.stateMoneyV = _this.searchDayV == today.getDate() ? 1 : 2、、测试用，可删除
                  }
    ...
    demo() {
      this.$refs.Calendars.ChoseMonth('2018-12-12'); //跳到12月12日选中12月12日
    },
    mrDateClick(){
